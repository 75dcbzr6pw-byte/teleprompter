(() => {
  ...
  function wordsPerMinute(level=settings.speed) { return Math.round(80+(Math.max(1,Math.min(20,Number(level)))-1)*(250-80)/19); }
  function updateSpeedDisplay(){ const level=Number(settings.speed); const ppm=wordsPerMinute(level); $('speedOutput').value=`${level} · ${ppm} ppm`; $('speedSlider').setAttribute('aria-valuetext',`Nivel ${level}, ${ppm} palabras por minuto`); }
  ...
  function resetPrompter(){ stopScroll(); countdownActive=false; clearInterval(countdownTimer); $('countdownOverlay').hidden=true; $('scrollViewport').scrollTop=0; $('startButton').textContent='Iniciar'; $('controls').classList.remove('hidden'); }
  function stopScroll(){ scrolling=false; cancelAnimationFrame(raf); raf=0; lastFrame=0; $('startButton').textContent='Continuar'; releaseWakeLock(); }
  ...
  function scrollRate(){ const viewport=$('scrollViewport'); const distance=Math.max(1,viewport.scrollHeight-viewport.clientHeight); const words=Math.max(1,$('prompterText').textContent.trim().split(/\s+/).length); return distance/(words/wordsPerMinute()*60); }
  function step(now){ if(!scrolling)return; const dt=Math.min((now-lastFrame)/1000,.05);lastFrame=now;const viewport=$('scrollViewport');viewport.scrollTop+=scrollRate()*dt;if(viewport.scrollTop+viewport.clientHeight>=viewport.scrollHeight-2){stopScroll();$('startButton').textContent='Finalizado';$('controls').classList.remove('hidden');return;}raf=requestAnimationFrame(step); }
